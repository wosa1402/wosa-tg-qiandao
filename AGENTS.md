...
- `tg_signer/` main package: `__main__.py` entrypoint, `cli/` (Click commands for signer/monitor flows), `core.py` scheduling & action logic, `config.py` config models, `notification/` (server_chan hooks), and shared helpers in `utils.py`.
...
- Run CLI: `python -m tg_signer --help` or `tg-signer run` after install.
...
- Run `pytest -vv` before PRs; add targeted cases when adding CLI flags or core behaviors.

## Commit & Pull Request Guidelines
- Commit style mirrors history (`feat(cli): ...`, `fix(core): ...`, `chore:`); include scope when helpful and optional issue refs `(#123)`.
- Keep commits focused and narrative (what/why). Do not commit secrets or local session/log/db artifacts.
- PRs should outline behavior changes, tests run, and UX impact (CLI snippets when relevant).
...
- 需要记录项目使用说明文档、修改记录文档、技术文档、测试指南文档
- 代码注释使用中文
- API 文档用中文编写
- 技术文档用中文撰写
- 项目使用说明文档用中文说明
...
- win系统本地开发，请不要运行任何测试
- 如果需要测试，只需要在测试文档中指导用户测试，不要在本地测试，本地环境只用于代码开发